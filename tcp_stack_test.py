...
if __name__ == "__main__":
    ...
    parser.add_argument("--logdir", type=str, metavar="path",
                        help="Where to place output log files."
                             "If not specified, will use configuration from "
                             "config.yml")
    parser.add_argument("--server_cores", type=str, metavar="x-y",
                        help="Range of CPU cores to use for server processes."
                             "If not specified, will use all available cores.")
    parser.add_argument("--client_cores", type=str, metavar="x-y",
                        help="Range of CPU cores to use for client processes."
                             "If not specified, will reuse server config.")
    args = parser.parse_args()
    if args.no_vpp:
        USE_VPP = False
    if args.logdir:
        logdir = args.logdir
    if args.server_cores:
        try:
            lowcore, highcore = args.server_cores.split("-")
            lowcore = int(lowcore)
    ...
    if args.client_cores:
        try:
            lowcore, highcore = args.client_cores.split("-")
            lowcore = int(lowcore)
            highcore = int(highcore)
        except ValueError:
            raise ValueError("Invalid value for 'cores' argument, must be "
        ...
        if int(highcore) >= cpu_count():
            raise ValueError(
                ...
                " specified {2} as last core.".format(
                    ...
                    cpu_count() - 1,
                    highcore))
        corelist_client = [x for x in range(int(lowcore), int(highcore) + 1)]
